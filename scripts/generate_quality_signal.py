...
def render(summary: dict, theme_name: str) -> str:
    ...
    parts.append(
        f'<text x="32" y="232" font-family="{MONO}" font-size="11" fill="{theme["muted"]}">'
    ...
    for count in summary["per_day"]:
        if spark_max > 0 and count > 0:
            height = 3 + round(44 * count / spark_max)
            color = theme["accent2"]
        else:
            height = 3
            color = theme["bar_bg"]
        parts.append(
            f'<rect x="{x}" y="{286 - height}" width="26" height="{height}" rx="2" fill="{color}"/>'
        )
        x += 36
